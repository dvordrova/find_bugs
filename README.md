...
- [concurrency/message_order_assumption](concurrency/message_order_assumption/README.md): a read-model projector assumes related messages arrive in one order. An order-permutation test exposes the lost update when messages are reordered.
...
- [golangci/http_response_body_leak](golangci/http_response_body_leak/README.md): an HTTP client reads the status code but forgets to close `resp.Body`. `bodyclose` through `golangci-lint` reports the connection leak.
...
- [teamrules/no_infrastructure_imports_in_domain](teamrules/no_infrastructure_imports_in_domain/README.md): domain code imports `database/sql` and `net/http`. `depguard` keeps infrastructure dependencies out of domain packages.
- [teamrules/context_first_argument](teamrules/context_first_argument/README.md): service code accepts `context.Context` after a business argument. revive's `context-as-argument` rule keeps context first.
