...
- [concurrency/message_order_assumption](concurrency/message_order_assumption/README.ru.md): read-model projector считает, что связанные messages приходят в одном порядке. Order-permutation test показывает lost update при reordered delivery.
- [concurrency/select_priority_assumption](concurrency/select_priority_assumption/README.ru.md): dispatcher считает, что первый ready `select` case имеет priority. Repeated schedule check показывает, что Go `select` может выбрать другой ready case.
...
- [golangci/http_response_body_leak](golangci/http_response_body_leak/README.ru.md): HTTP client читает status code, но забывает закрыть `resp.Body`. `bodyclose` через `golangci-lint` репортит connection leak.
...
- [teamrules/force_sqlc_query_layer](teamrules/force_sqlc_query_layer/README.ru.md): repository code пишет raw SQL через `*sql.DB`, хотя generated query layer уже есть. Type-aware правило `ruleguard` держит database calls внутри sqlc packages.
- [teamrules/transaction_boundary](teamrules/transaction_boundary/README.ru.md): service code сам начинает, откатывает и коммитит transaction. Type-aware правило `ruleguard` держит transaction lifecycle внутри transaction manager packages.
- [teamrules/no_infrastructure_imports_in_domain](teamrules/no_infrastructure_imports_in_domain/README.ru.md): domain code импортирует `database/sql` и `net/http`. `depguard` держит infrastructure dependencies вне domain packages.
- [teamrules/context_first_argument](teamrules/context_first_argument/README.ru.md): service code принимает `context.Context` после business argument. Revive rule `context-as-argument` держит context первым.
...
- [synctest/unbuffered_send_after_timeout](synctest/unbuffered_send_after_timeout/README.ru.md): timeout возвращается caller, а worker legacy client позже блокируется на unbuffered result send. `testing/synctest` показывает blocked goroutine без wall-clock sleeps.
